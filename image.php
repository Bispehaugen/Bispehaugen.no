...
function thumbnail($filepath, $size = 100, $crop = 1, $trim = 0, $zoom = 0, $align = false, $sharpen = 0, $gray = 0, $ignore = 0) {

    $src = isset($filepath) ? $filepath : false;
    $size = isset($size) ? str_replace(array('<', 'x'), '', $size) != '' ? $size : 100 : 100;
    $crop = isset($crop) ? max(0, min(1, $crop)) : 1;
    $trim = isset($trim) ? max(0, min(1, $trim)) : 0;
    $zoom = isset($zoom) ? max(0, min(1, $zoom)) : 0;
    $align = isset($align) ? $align : false;
    $sharpen = isset($sharpen) ? max(0, min(100, $sharpen)) : 0;
    $gray = isset($gray) ? max(0, min(1, $gray)) : 0;
    $ignore = isset($ignore) ? max(0, min(1, $ignore)) : 0;
    $path = parse_url($src);
