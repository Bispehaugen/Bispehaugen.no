...
function koble_til_database($database_host, $database_user, $database_string, $database_database){
    ...
    if (!$connection) {
        echo "Kunne ikke koble opp mot database.<br>Pr�v igjen senere...";
        return false;
    }
...
function inkluder_side_fra_undermappe($sidenavn = "forside", $mappenavn = "sider"){
	...
	// hvis $page inneholder .. eller / s� pr�ver noen � g� i undermapper, det vil vi ikke
...
function list_forum(){
    ...
    <td><a href=''>p�meldinger</a></td>";  
    if($_SESSION['rettigheter']>1){
		echo"
    	<td><a href=''>musikkomit�en</a></td>
...
function ant_dager_siden($dato){
	//dager siden siste innlegg
			...
			elseif($dagersiden<31){
				...
				$dagersiden_som_tekst = " for ".floor($dagersiden/30)." m�neder siden";
			}else{
