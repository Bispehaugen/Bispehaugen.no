<?php 
	...
	$sql="SELECT tittel, forumid, pos, sisteinnleggskrevetav, sisteinnleggskrevet FROM forum ORDER BY pos;";
    ...
    echo "<table><tr><th></th><th>Forum</th><th>Sist oppdatert av</th></tr>";
   	...
   	foreach($forumer as $forum){
   		...
   		if($_SESSION['rettigheter']>2 || $forum['forumid']<3){
   			...
   			echo "<tr><td></td><td><a href='?side=forum/tema&id=".$forum['forumid']."'>".$forum['tittel']."</a></td><td>".$forum['sisteinnleggskrevetav'];
   			if ($dagersiden==0){
   				echo" i dag</td></tr>";
   			}
			elseif ($dagersiden==1){
