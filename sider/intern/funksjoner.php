...
function neste_kakebaking() {
	...
	$sql = "SELECT arrid, tittel, dato, oppmoetetid FROM `arrangement` WHERE kakebaker = '".$bruker['medlemsid']."' AND slettet = 0 AND start > NOW() ORDER BY `start` DESC LIMIT 1";
	return hent_og_putt_inn_i_array($sql);
}
...
function neste_slagverkhjelp() {
	...
	$sql = "SELECT arrid, tittel, dato, oppmoetetid, slagverk FROM `arrangement` WHERE slagverk = '".$gruppe['gruppeid']."' AND slettet = 0 AND start > NOW() ORDER BY `start` DESC LIMIT 1";
	return hent_og_putt_inn_i_array($sql);
}